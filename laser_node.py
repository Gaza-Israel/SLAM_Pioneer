...
from Feature_detector import feature_detector, feature_matcher,polar2z,z2polar
...
class Laser_node:
    ...
    def extract_laser_data_from_message(self,data:LaserScan):
        ...
        array_has_nan = np.logical_or(np.isnan(data.ranges), data.ranges < 0.05)
        data.ranges = data.ranges[np.invert(array_has_nan)]
    ...
    def laser_scan_callback(self,data,*args):
        map_landmarks = args[0]
        pose = args[1].astype(PoseWithCovarianceStamped)
    ...
    def Map_features_callback(self,data:Marker,pointer):
        ...
        for point in data.points:
            pointer.append([point.x,point.y])
